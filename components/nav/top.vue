<script setup>
	...
	async function fetchUserData() {
		...
		} else if (route.path.includes('/projects') && !route.params.id) {
			title.value = 'Motis Group';
			description.value = 'Projects';
		} else if (
			route.path.includes('/org') &&
			route.params.organization
		) {
			title.value = 'Motis Group';
			description.value = 'Settings';
		} else if (route.path.includes('/account/me')) {
			title.value = 'Motis Group';
...
<template>
	<div
		class="border-scale-500 sticky top-0 z-50 flex h-12 max-h-12 items-center justify-between border-b bg-[#F8F8FB] px-5 py-3 dark:border-white/10 dark:bg-[#020014]"
	>
		...
		<div class="flex items-center space-x-2">
			<Menu as="div" class="relative"
				><MenuButton
					...
					data-state="closed"
					class="focus:outline-scale-600 flex rounded p-0 outline-none outline-offset-1 transition-all focus:outline-4"
				>
					<span
