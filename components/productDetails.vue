...
<template>
	<div class="mx-auto mb-16 h-full px-4 sm:px-6 lg:px-8">
		<!-- Product -->
		<div
			...
			>
					...
					<div class="">
						...
						<div class="mx-auto mt-4 w-full max-w-lg space-y-8 lg:mx-0">
							<div class="">
								...
								<p class="grid grid-cols-5 space-x-3">
									<span class="flex flex-col">
										<span
											...
											>{{
												product.setup_fee === 0
													? 'NO'
													: `$${product.setup_fee.toLocaleString()}`
											}}</span
										>
										<span
              ...
              >
				...
				<div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-4">
					<button
						:disabled="exists || (workspace)"
						@click="workspace ? handleCheckout(product, workspace) : "
						type="button"
						:class="[
							exists ? 'bg-gray-500' : 'bg-indigo-600 hover:bg-indigo-700',
					...
					>
						{{
							exists || !workspace
								? 'Downloaded'
								: `Pay $${product.setup_fee.toLocaleString()}`
						}}
					</button>
...
<script>
	export default {
		props: ['product', 'workspace', 'exists', 'user'],
