						...
						<table class="table">
							<thead>
								<tr>
									...
									<th class="p-3 px-4 text-left">Payout number</th>
							...
							<tbody>
								...
								<tr class="" v-if="invoices.length === 0 && !loading">
									<td class="col-span-6 p-3 py-12 text-center" colspan="6">
								...
								<tr class="" v-else>
									<td class="col-span-6 p-3 py-12 text-center" colspan="6">
										<p class="text-scale-1000 flex items-center justify-center">
											<svg
												xmlns="http://www.w3.org/2000/svg"
...
<script setup>
	...
	const getInvoices = async () => {
		...
		const { data } = await $fetch(
			`/api/stripe/transfers/${accountData.stripe_account_id}`
		);
		return data;
	};
